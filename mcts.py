...
class State():
    ...
    def select(self, c=1.5, uct=False):
        ...
        if uct:
            UCT = np.array(
                [child_action.Q + c * (np.sqrt(np.log(self.n + 1e-5) / (child_action.n + 1))) for child_action in
                 self.child_actions])
        else:
            UCT = np.array(
...
class MCTS():
    ...
    def __init__(self, root, root_index, model, na, gamma, dpw=False, alpha=0.6, uct=False):
        self.root = None
