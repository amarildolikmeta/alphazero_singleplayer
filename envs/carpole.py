...
class CartPoleEnv(gym.Env):
    ...
    def __init__(self, horizon=100, fail_prob=0.05):
    ...
    def step(self, action):
        ...
        if self.noise >= 0:
            self.state[0] += self.np_random.uniform(low=-self.noise, high=self.noise)
            self.state[2] += self.np_random.uniform(low=-self.noise, high=self.noise)
        ...
        if not done:
            ...
            reward = 0.0
        self._t += 1
        self.done = done or self._t >= self.horizon
