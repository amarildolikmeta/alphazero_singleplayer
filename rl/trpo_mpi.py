import os
from rl.common import explained_variance, zipsame, dataset
...
from rl.common.mpi_adam import MpiAdam
from rl.common.cg import cg
from rl.common.input import observation_placeholder
from rl.common.policies import build_policy
...
def traj_segment_generator(pi, env, horizon, stochastic, gamma=0.99):
    ...
    while True:
        ...
        if t > 0 and t % horizon == 0:
            yield {"ob": obs, "rew": rews, "vpred": vpreds, "new": news,
                   ...
                   "ep_rets": ep_rets, "ep_disc_rets": ep_disc_rets, "ep_lens": ep_lens}
        ...
        cur_ep_ret += rew
        cur_ep_disc_ret += gamma**cur_ep_len * rew
        cur_ep_len += 1
        if new:
            ep_rets.append(cur_ep_ret)
            ep_lens.append(cur_ep_len)
            ep_disc_rets.append(cur_ep_disc_ret)
            cur_ep_ret = 0
            cur_ep_disc_ret = 0
            cur_ep_len = 0
            ob = env.reset()
    ...
    **network_kwargs        keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    while True:
        ...
        if iters_so_far % checkpoint_freq == 0 and checkpoint_dir_out is not None:
            ...
            logger.log(
                'Saved policy weights as %s' % os.path.join(checkpoint_dir_out, 'checkpoint_%d.npy' % iters_so_far))

            def pi_wrapper(ob):
                ac, vpred, _, _ = pi.step(ob, stochastic=True)
                return ac

            rew, _, logs, disc_rets, num_stops, avg_damages = eval_policy(pi=pi_wrapper, n_episodes=eval_episodes, verbose=True)
            offline_scores.append([np.mean(disc_rets), np.mean(num_stops), np.mean(avg_damages)])
        ...
        np.save(os.path.join(checkpoint_dir_out, 'online_scores.npy'), online_scores)
        # Saving best
