...
class OLOP(AbstractPlanner):
    ...
    def horizon(self, episodes, gamma):
        return np.clip(max(int(np.ceil(np.log(episodes) / (2 * np.log(1 / gamma)))), 1), None, self.config['max_depth'])
    ...
    def allocate_budget(self):
        ...
        for episodes in range(1, int(self.config["budget"])):
            if episodes * self.horizon(episodes, self.config["gamma"]) > self.config["budget"]:
                self.config["episodes"] = max(episodes - 1, 1)
                self.config["horizon"] = self.horizon(self.config["episodes"], self.config["gamma"])
                break
        else:
