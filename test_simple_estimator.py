...
class Estimator:
    ...
    def check_sub_trajectory(self, nu, distribution, depth):
        for particle in distribution:
            t = depth - 1
            p = nu[t]
            while particle.state == p.state and particle.reward == p.reward and particle.parent_particle is not None:
            ...
            if sample_depth == 0:
                ...
                for j in range(T):
                    ...
                    if depths[j] > sample_depth:
                        ...
                        p = self.check_sub_trajectory(nu, distributions[j][0][:num_particles_j], depths[j])
                        if p is not None:
                            if depths[j] == sample_depth:
                                p_nu_j = 1
                            else:
    ...
    def should_resample(self, node, bh=False):
        ...
        if bh:
            ...
            full_resampling_weights = self.get_new_weights_balance_heuristic(trajectory=[self.root_particle],
                                                                             ...
                                                                             distribution=([self.root_particle], 1))
        else:
            ...
            #full_resampling_weights = np.array(full_resampling_weights) / np.sum(full_resampling_weights)
    ...
    def backup(self, node, particle, depth, weight, sampled_particle, sampled_node, bh=False):
        #add trajectory to list of samples
        if bh:
            last_node = node
            ...
            # weights = np.array(self.weights) / len(self.weights)
        self.ess = self.compute_ess(weights)
        R = 0
        while node.parent_node is not None:
            if not bh:
    ...
    def resample_from_particle(self, node, particle, weight, depth, bh=False):
        ...
        for i in range(n):
            ...
            while remaining_budget > 0:
                ...
                if j == 0:
                    ...
                    j = 1
                    last_node = node
                    self.backup(last_node, particle, sampled_node=root, depth=0, sampled_particle=root_particle,
                                bh=bh, weight=1)
                    ...
                    while node.parent_node is not None:
                        if np.random.random() < resample_prob:
                            should_resample, particle, weight, margin = self.should_resample(node, bh=bh)
                            if should_resample:
                                resampled = True
                    ...
                    generated_particle = self.resample_from_particle(node=node, particle=starting_particle,
                                                                     weight=sample_weight, depth=sample_depth, bh=bh)
                    remaining_budget -= (self.full_cost - sample_depth)
                    self.backup(self.last_node, generated_particle, bh=bh, depth=sample_depth, sampled_node=node,
            ...
            counts.append(count)
            ess.append(self.compute_ess(weights))
...
if __name__ == '__main__':
    ...
    #
    print("Doing " + str(n) + " Particle Simple estimations with " + str(budget) + " samples")
    ...
    print("Doing " + str(n) + " Particle BH estimations with " + str(budget) + " samples")
    estimations_particle_bh, ess_bh, depths_bh, counts_bh = estimator.run_particle_estimation(n, budget, bh=True)
    pyplot.hist(estimations_particle_bh, bins, alpha=0.5, label='PARTICLE BH', density=True)
    ...
    pyplot.show()
    #
    pyplot.hist(ess, bins, alpha=0.5, label='ess_simple')
    pyplot.hist(ess_bh, bins, alpha=0.5, label='ess_bh')
    pyplot.legend(loc='upper right')
    pyplot.show()

    pyplot.plot(np.arange(action_length), depths, alpha=0.5, label='depths_simple')
    pyplot.plot(np.arange(action_length), depths_bh, alpha=0.5, label='depths_bh')
    pyplot.legend(loc='upper right')
    pyplot.show()

    pyplot.hist(counts, bins, alpha=0.5, label='particle simple samples')
    pyplot.hist(counts_bh, bins, alpha=0.5, label='particle bh samples')
    pyplot.legend(loc='upper right')
    pyplot.show()
    ...
    for b in budgets:
        ...
        estimations_particle, ess, _, counts = estimator.run_particle_estimation(n, b, resample_prob=1., bh=bh)
